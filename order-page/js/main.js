...
jQuery(function ($) {
	...
	$('#sendOrder').click(function (e) {
		...
		let care = orderCare;
		let sendData = {
			first_name: surname.val(),
			last_name: name.val(),
			tel_number: userTel.val(),
			email: userEmail.val(),
			type_cleaning: care,
			date_cleaning: date.val(),
			flowers: flowers,
			qr: qr,
			...
			cementry_first_name: userSurname.val(),
			cementry_last_name: userName.val(),
			cementry_patronymic: patronymic.val()
		};
		let sendHelp = {
			cementry_city: city.val(),
			last_name: userHelpName.val(),
			tel_number: userHelpTel.val(),
			email: userHelpEmail.val(),
			type_cleaning: care,
			date_cleaning: date.val(),
			flowers: flowers,
			qr: qr
		...
		if ($('#yes').is(':checked')) {
			if (sector.val() == '' || area.val() == '' || row.val() == '' || number.val() == '' || validateName(name) == false || validateTel(userTel) == false || validateName(surname) == false || validateName(patronymic) == false || validateName(userSurname) == false || validateName(userName) == false || validateMail(userEmail) == false || validateName(cemetery) == false) {
				showErrorSuccess("Данные не заполнены или введены не верно!", 1500);
				setTimeout(function () {
					cemetery.focus();
				}, 2000);
				return false;
				...
				showErrorSuccess("Выберите предварительную дату!", 1500)
			} else {
				fetch(
					`${api_url}send_order`,
					{
					...
					.then(json => {
						if (json.error == 0) {
							showErrorSuccess("Заказ отправлен!Мы скоро свяжемся с вами!", 1500);
