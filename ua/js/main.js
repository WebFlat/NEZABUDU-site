...
$(document).ready(function () {
    ...
    function start() {
        fetch(
            ...
            .catch(error => {
                ...
                showErrorSuccess('Authorization error', 2000);
    ...
    function validateMail() {
        ...
        if (reg.test(regEmail) == false || regEmail == '') {
            showErrorSuccess("Введіть правильну електронну адресу", 2000);
            return false;
        } else {
            return true;
        }
    ...
    function validatePatronymic() {
        ...
        if (patronymic == '') {
            showErrorSuccess("Введіть своє ім'я по батькові", 2000);
            return false;
        } else {
            return true;
        }
    ...
    function validateTel() {
        ...
        var tel = $('#reg-tel').val();
        if (tel == '') {
            showErrorSuccess("Введіть правильний номер телефону", 2000);
            return false;
        } else {
            return true;
        }
    ...
    function validatePass() {
        ...
        if (pass == '' || pass.length < 6) {
            showErrorSuccess("Введіть пароль щонайменше 6 символів", 2000);
            return false;
        } else {
            return true;
        }
    ...
    $('#remember').click(function () {
        ...
        if (validateMail(email)) {
            fetch(
                ...
                .then(json => {
                    if (json.error == 0) {
                        showErrorSuccess("Пароль надіслано на електронну пошту", 2000);
                    } else {
            ...
            showErrorSuccess('Спочатку введіть електронну адресу', 2000);
        }
    });
        ...
        for (let key in data) {
            ...
            if (data[key].death_date && count < 11) {
                if (isFamous) {
                    ...
                    <div class="open-profile-button button">Читати далі</div>
                    </a>`;
                    count++;
                } else {
                    out2 += `<a href="../questionnaire-life/#${data[key].id}" class="famous-profile-${key} famous-profile swiper-slide">
    ...
    let work_type = [
        "Робота з музеями",
        "Написання статей про особистості",
        "Опитування ветеранів",
        "Прибирання могил ветеранів",
        "Покладання квітів ветеранам",
        "Співпраця з державними організаціями",
        "Співпраця з громадами",
        "Робота з прибирання на цвинтарі",
        "Оцифровка архівів",
        "Оцифровка даних з пам’яток",
        "Оцифрування даних із пам'ятників",
        "Партнерство з догляду за похованнями"
    ];
...
$('.volunteer-form').submit(function (e) {
    ...
    if (validateName() == false || validateTel() == false) {
        showErrorSuccess("Введені дані невірні!", 2000);
    } else {
        fetch(
            ...
            .then(json => {
                if (json.error == 0) {
                    showErrorSuccess("Заявку надіслано, ми зв'яжемося з вами найближчим часом", 2000);
                } else {
                    showErrorSuccess("Помилка відправки", 2000);
                }
