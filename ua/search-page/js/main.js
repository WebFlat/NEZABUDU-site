		...
		fetch(
			...
			.then(json => {
				if (json.error == 0) {
					...
					showErrorSuccess("Цей користувач вже існує", 2000);
					clearInput();
				}
			...
			.catch(error => {
				...
				showErrorSuccess("Помилка з'єднання", 2000);
			});
	} else {
		showErrorSuccess("Помилка з'єднання", 2000);
	};
...
jQuery(function ($) {
	...
	//Get profiles when load*******************************
	function getFirstProfileSearch() {
		let query = decodeURI(window.location.hash).slice(1);
		if (query) {
			getProfileSearch(query);
		} else {
			showErrorSuccess("Введіть дані для пошуку", 1000);
		}
	}


	//Get profile with search************************************************
	function getProfileSearch(queries) {
		fetch(
			`${api_url}search?search=${queries}`,
			{
				method: 'GET',
				headers: {
					//'Authorization': 'Token token=' + cookie_token,
			...
			.then(data => {
				...
				if (data.profiles.length == 0) {
					showErrorSuccess("Немає даних по вашому запиту", 2000);
				} else {
					loadQuestionnaries(data.profiles, wrapper);
				}
			})
			.catch(error => {
				console.error('error1:', error);
		...
		for (let key in data) {
			...
			if (data[key].death_date == null) {
				if (data[key].profile_mine == true) {
					out += '<div class="user item"><div class="user__info"><div class="user__avatar-wrap user__avatar-wrap--mine"><img data-src="' + data[key].avatar + '" alt="face" class="user__avatar lazyload"></div><div class="user__title"><span class="user__surname">' + data[key].first_name + '</span><span></span><span class="user__name">' + data[key].last_name + '</span><span class="user__patronymic">' + data[key].patronymic + '</span></div><div class="user__lives"><span class="user__both">' + birth + '</span><span></div><a href="../questionnaire-life/#' + data[key].id + '" class="user__link-more">Читати далі</a></div></div>';
				} else {
					out += '<div class="user item"><div class="user__info"><div class="user__avatar-wrap user__avatar-wrap--life"><img data-src="' + data[key].avatar + '" alt="face" class="user__avatar lazyload"></div><div class="user__title"><span class="user__surname">' + data[key].first_name + '</span><span></span><span class="user__name">' + data[key].last_name + '</span><span class="user__patronymic">' + data[key].patronymic + '</span></div><div class="user__lives"><span class="user__both">' + birth + '</span><span></div><a href="../questionnaire-life/#' + data[key].id + '" class="user__link-more">Читати далі</a></div></div>';
				}
			} else {
				out += '<div class="user  item"><div class="user__info"><div class="user__avatar-wrap user__avatar-wrap--death"><img data-src="' + data[key].avatar + '" alt="face" class="user__avatar lazyload"></div><div class="user__title"><span class="user__surname">' + data[key].first_name + '</span><span></span><span class="user__name">' + data[key].last_name + '</span><span class="user__patronymic">' + data[key].patronymic + '</span></div><div class="user__lives"><span class="user__both">' + birth + '</span><span> - </span><span class="user__die">' + die + '</span></div></div><div class="user__btns"><a href="../questionnaire-life/#' + data[key].id + '" class="user__link-more">Читати далі</a></div></div></div>';
			}
			section.html(out);
	...
	$('#remember').click(function () {
		...
		function validateMail(email) {
			...
			if (reg.test(email) == false || email == '') {
				showErrorSuccess('Введіть правильний e-mail', 2000);
				return false;
			} else {
				return true;
			}
		...
		if (validateMail(email)) {
			fetch(
				...
				.then(json => {
					if (json.error == 0) {
						showErrorSuccess("Пароль надіслано на пошту", 2000);
					} else {
						showErrorSuccess("Помилка надсилання", 2000);
						clearInput();
					}
				})
				.catch(error => {
			...
			showErrorSuccess('Введіть спочатку e-mail', 2000);
		}
	});
	...
	function validateSurname() {
		...
		if (surname == '') {
			showErrorSuccess('Введіть прізвище', 2000);
			return false;
		} else {
			return true;
		}
	...
	function validateName() {
		...
		if (name == '') {
			showErrorSuccess("Введіть ім'я", 2000);
			return false;
		} else {
			return true;
		}
	...
	function validatePatronymic() {
		...
		if (patronymic == '') {
			showErrorSuccess('Введіть по-батькові', 2000);
			return false;
		} else {
			return true;
		}
	...
	function validateTel() {
		...
		var tel = $('#reg-tel').val();
		if (tel == '') {
			showErrorSuccess('Введіть правильний телефон', 2000);
			return false;
		} else {
			return true;
		}
	...
	function validatePass() {
		...
		if (pass == '' || pass.length < 6) {
			showErrorSuccess('Введіть правильний пароль мін 6 символів', 2000);
			return false;
		} else {
			return true;
		}
			...
			fetch(
				...
				.then(json => {
					...
					if (json.error == 0) {
						...
						showErrorSuccess('Такий користувач вже існує', 2000);
						clearInput();
					}
	...
	$('#authSend').click(function (e) {
		...
		if (authPassword === '') {
			showErrorSuccess('Введіть пароль', 2000);
		}
				...
				fetch(
					...
					.then(json => {
						...
						if (typeof json.token !== 'undefined') {
							...
							showErrorSuccess('Перевірте логін та пароль', 2000);
							clearInput();
						}
	...
	// });
	// $('.search-person__btn-showHide').click(function () {
	// 	$('.search-person__more-wrap').toggleClass('show-select');
	// 	$('.search-person__btn-showHide').toggleClass('active-btn');
	// });
		...
		function goToLanguage(arr) {
			for (let i = 0; i < arr.length; i++) {
				arr[i].addEventListener('click', () => {
					...
					switch (key) {
						case 'Русский':
							window.location.href = 'https://nezabudu.com/search-page/';
							break;
						case 'Українська':
							window.location.href = 'https://nezabudu.com/ua/search-page/';
							break;
						case 'English':
							window.location.href = 'https://nezabudu.com/en/search-page/';
							break;
					}
				})
			}
